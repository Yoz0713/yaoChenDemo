$(document).ready(function() {
  ...
  $(window).bind('scroll resize', function() {
    ...
    if ($(window).width()<=768) {
      ...
      $('#pc_life_more').click(function(event) {
        ...
        //-- 判斷有無新聞內容 --
        if($('#news_div .news_list ul').html().indexOf('li')==-1){
          $('#news_pc_btn').css('display', 'none');
        }

        //-- 電腦 --
        if($(window).width()>1024){
          $.each($('#news_div .news_list a'), function(index, val) {
             $(this).attr('href', $(this).attr('data-src'));
          });
          $('#news_div .news_list a').removeAttr('data-fancybox');
          $('#news_div .news_list a').removeAttr('data-type');
          $('#news_div .news_list a').attr('target', '_blank');
          $('#news_div .news_list a').removeAttr('data-src');
        }
    ...
    //-- 手機平板 --
       ...
       $('#navbarNavAltMarkup a').click(function(event) {
        if ($(this).attr('anchor')!=null) {
          var anchor_offset=$('#'+$(this).attr('anchor')).offset().top-42; //錨點位置+位移量-body位移量-navbar高度
    ...
    //-------------- 聯絡我們 -------------
     if ($('.contact_us').length>0) {
       ...
       $('#sub_btn').click(function(event) {
        ...
        e.preventDefault();
        let err_arr=[
          // {DOM_id:'#ca_name', txt:'全名，'},
          // {DOM_id:'#ca_phone', txt:'電話號碼，'},
        ];

        $.each($('input.required'), function (index, valueOfElement) { 
          err_arr.push({DOM_id:$(this).attr('id'), txt:`${$(this).attr('err_name')}，`});
        });

        let ca_name=$('#ca_name').length>0 ? $('#ca_name').val() : '';
        let ca_phone=$('#ca_phone').length>0 ? $('#ca_phone').val() : '';
        let ca_email=$('#ca_email').length>0 ? $('#ca_email').val() : '';
        let msg='';

        $.each($('.contact_us_div input[type="text"], .contact_us_div input[type="radio"]:checked, .contact_us_div select, .contact_us_div textarea'), function (index, valueOfElement) { 
          if($(this).attr('type')!='hidden' && $(this).attr('id')!='ca_name' && $(this).attr('id')!='ca_phone' && $(this).attr('id')!='ca_email' && $(this).attr('id')!='ca_privacy'){
            msg+=`${$(this).attr('input_name')}：${$(this).val()}｜`;
          }
        });
        ...
        let ajax_data={
            ...
            phone: ca_phone,
            mail: ca_email,
            //msg:  `預算：${$('#ca_money').val()}｜購屋需求：${$('#ca_need').val()}｜年齡：${$('#ca_year').val()}｜目前居住區域：${$('#ca_area').val()}｜方便聯繫時間：${$('#ca_time').val()}`,
            msg:  msg,
        ...
        form_submit(err_arr, ajax_data, $('#thanks_url').val());
        ...
        //  // }
        //  else{
        //    ga('send', 'event', $('.contact_us h1').html(), 'click', 'from');
  ...
  // var qrcode = new QRCode(document.getElementById('qr_code_one'), {
  //   width: 250,
  //   height: 250
  // });

  // qrcode.makeCode(web_URL);
...
function form_template(get_url) {
  let jqxhr= $.get(get_url, function (data, textStatus, jqXHR) {
 ...
 * @param {String} get_url 樣板網址
 */
 function form_template_new (get_url, DOM='#call_form') {
  let jqxhr= $.get(get_url, function (data, textStatus, jqXHR) {
              ...
              $(DOM).html(data);
              $(DOM).append(send_list);
              $(DOM).append(case_id);
              $(DOM).append(case_aes_id);
              $(DOM).append(test);
            });
  return jqxhr;
}
 ...
 * @param {string} loc_url 跳轉連結
 */
function form_submit(err_obj, ajax_data, loc_url='back', loc_txt='感謝您的來信預約，我們將有專人與您聯繫') {
